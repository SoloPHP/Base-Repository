...
| `$autoIncrement` | `bool` | `true` | Whether the PK is generated by the database (AUTO_INCREMENT / SERIAL / IDENTITY). Set to `false` for tables with manual PK (UUID, ULID, prefixed IDs, etc.) — `create()` then requires the PK in `$data` and skips `lastInsertId()` |
